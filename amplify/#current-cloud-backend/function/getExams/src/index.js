...
exports.handler = async (event) => {
  const PAGARME_API_KEY = process.env.ENV === 'prd' ? PAGARME_API_KEY_DEV : PAGARME_API_KEY_PRD;
  try {
    const res = await pagarmeAPI.get(`/transactions?count=100&customer[external_id]=${event.arguments.username}&api_key=${PAGARME_API_KEY}`);
